export default class CvResourcesHeroComponent {
  constructor(rootId) {
    this.root = document.getElementById(rootId);

    if (!this.root) {
      console.error(`CV Resources Hero root introuvable: ${rootId}`);
      return;
    }
  ...
  render() {
    this.root.innerHTML = `
      <section class="w-full border-t border-white/60 bg-[linear-gradient(180deg,#f0ffff_0%,#f8f8ff_100%)] py-16 sm:py-20">
...
const AUTO_ROOT_ID = "sierra-cv-resources-hero-root";
if (document.getElementById(AUTO_ROOT_ID)) {
  new CvResourcesHeroComponent(AUTO_ROOT_ID);
}
