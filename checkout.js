...
class CheckoutModal {
  constructor(options = {}) {
    this.options = {
      ...
      client: null,
      depositMode: false,
      lockDepositAmount: false,
      depositLabel: 'Depot sur votre compte',
      depositDescription: 'Choisissez le montant a deposer, puis confirmez le paiement.',
      minDepositAmount: 5,
    ...
    this.depositMode = Boolean(this.options.depositMode);
    ...
    if (this.depositMode) {
      this.setDepositAmount(
        this.options.defaultDepositAmount || this.cart[0]?.price || this.options.minDepositAmount,
        { silent: true }
      );
    }
  ...
  setDepositAmount(value, { silent = false } = {}) {
    const minimum = Number(this.options.minDepositAmount) > 0 ? Number(this.options.minDepositAmount) : 1;
    const lockedAmount = Number(this.options.defaultDepositAmount) > 0
      ? Number(this.options.defaultDepositAmount)
      : minimum;

    if (this.options.lockDepositAmount) {
      this.depositAmount = Math.max(minimum, Math.round(lockedAmount * 100) / 100);
    } else {
      const rawAmount = Number(String(value ?? '').replace(',', '.'));
      const normalized = Number.isFinite(rawAmount) && rawAmount > 0 ? rawAmount : minimum;
      this.depositAmount = Math.max(minimum, Math.round(normalized * 100) / 100);
    }

    this.cart = [
      {
        productId: 'wallet-deposit',
        name: this.options.depositLabel || 'Depot sur votre compte',
  ...
  renderDepositContent() {
    const presetAmounts = Array.isArray(this.options.presetAmounts) && this.options.presetAmounts.length
      ? this.options.presetAmounts
      : [10, 25, 50, 100];
    const isLocked = Boolean(this.options.lockDepositAmount);
    ...
    return `
      ...
      ">
        <h3 style="
          ...
          color: #1F1E1C;
        ">
          ${this.options.depositLabel || 'Depot sur votre compte'}
        </h3>
        <p style="color: #8B7E6B; margin-bottom: 1rem;">
          ${this.options.depositDescription || 'Choisissez le montant a deposer, puis confirmez le paiement.'}
        </p>
        ...
        ${isLocked ? `
          <div style="
            ...
            margin-bottom:0.5rem;
          ">
            <p style="font-size:0.8rem; color:#8B7E6B; text-transform:uppercase; letter-spacing:0.08em;">Montant fixe</p>
            <p style="margin-top:0.35rem; font-size:1.8rem; font-weight:700; color:#1F1E1C;">${this.formatPrice(this.depositAmount)}</p>
            <p style="margin-top:0.35rem; font-size:0.85rem; color:#8B7E6B;">Ce montant est fixe pour le paiement d un CV professionnel.</p>
          </div>
        ` : `
          <label style="display:block; font-size:0.9rem; color:#8B7E6B; margin-bottom:0.4rem;">
            Montant du depot
          </label>
          <input
            type="number"
            min="${Number(this.options.minDepositAmount) || 1}"
            step="1"
          ...
          <div style="display:flex; flex-wrap:wrap; gap:0.5rem;">
            ${presetAmounts.map((amount) => `
              <button
                type="button"
                data-deposit-preset="${amount}"
                style="
                  border:1px solid ${Number(amount) === Number(this.depositAmount) ? '#C6A75E' : 'rgba(198,167,94,0.25)'};
                  background:${Number(amount) === Number(this.depositAmount) ? '#C6A75E' : 'white'};
                  color:${Number(amount) === Number(this.depositAmount) ? '#1F1E1C' : '#8B7E6B'};
      ...
      ">
        ...
        ">
          ...
          ">
            ...
            <span data-subtotal-amount style="font-weight:500;">${this.formatPrice(this.subtotal)}</span>
          </div>
        ...
        " onmouseover="this.style.background='#C6A75E'; this.style.color='#1F1E1C'" onmouseout="this.style.background='#1F1E1C'; this.style.color='#F5F1E8'">
          ...
          <span data-pay-now-label>Payer ${this.formatPrice(this.total)}</span>
        </button>
      </div>
    `;
  }
  ...
  bindDepositEvents() {
    ...
    const amountInput = this.modal.querySelector('.deposit-amount-input');
    if (amountInput) {
      amountInput.addEventListener('input', () => {
        this.setDepositAmount(amountInput.value);
      });
    ...
    const presetButtons = this.modal.querySelectorAll('[data-deposit-preset]');
    presetButtons.forEach((button) => {
      button.addEventListener('click', () => {
        this.setDepositAmount(button.getAttribute('data-deposit-preset'));
      });
    });
  }

  updateDepositUi() {
    if (!this.depositMode || !this.modal) {
      return;
    }

    const amountInput = this.modal.querySelector('.deposit-amount-input');
    if (amountInput) {
      amountInput.value = String(this.depositAmount);
    }

    this.modal.querySelectorAll('[data-deposit-preset]').forEach((button) => {
      const buttonAmount = Number(button.getAttribute('data-deposit-preset'));
      const isActive = Number(buttonAmount) === Number(this.depositAmount);
      button.style.borderColor = isActive ? '#C6A75E' : 'rgba(198,167,94,0.25)';
      button.style.background = isActive ? '#C6A75E' : 'white';
      button.style.color = isActive ? '#1F1E1C' : '#8B7E6B';
    });

    const subtotalEl = this.modal.querySelector('[data-subtotal-amount]');
    if (subtotalEl) {
      subtotalEl.textContent = this.formatPrice(this.subtotal);
    }
    ...
    const payLabel = this.modal.querySelector('[data-pay-now-label]');
    if (payLabel) {
      payLabel.textContent = `Payer ${this.formatPrice(this.total)}`;
    }
  }
  ...
  async openPaymentModal() {
    try {
      ...
      if (this.depositMode && this.total <= 0) {
        this.showMessage('Veuillez saisir un montant valide.', 'error');
        return;
      }
      ...
      this.paymentModal = new PaymentModal({
        ...
        cart: this.cart,
        currency: this.options.currency,
        orderType: this.depositMode ? 'deposit' : 'order',
