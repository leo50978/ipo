...
const DEFAULT_PAYMENT_STEPS = [
  {
    ...
    fields: [
      ...
      { type: 'text', name: 'phone', label: 'Telephone', required: true }
    ]
  },
  {
    type: 'payment',
    title: 'Effectuez le paiement',
    instruction: 'Payez aux coordonnees suivantes :',
    buttonText: "J'ai paye"
  },
  {
    type: 'proof',
    title: 'Confirmez votre paiement',
    message: 'Telechargez une capture de votre transaction.',
...
function normalizePaymentStep(step = {}, fallbackStep = {}) {
  const type = ['form', 'payment', 'proof', 'confirmation', 'custom'].includes(step.type)
    ? step.type
    : fallbackStep.type || 'payment';
  const normalized = {
    ...fallbackStep,
    ...step,
    type
  ...
  if (type === 'form') {
    normalized.fields = Array.isArray(step.fields) && step.fields.length
      ? step.fields
      : (Array.isArray(fallbackStep.fields) ? fallbackStep.fields : []);
  }

  return normalized;
}

function resolveMethodSteps(method = {}) {
  const sourceSteps = Array.isArray(method.steps) && method.steps.length
    ? method.steps
    : DEFAULT_PAYMENT_STEPS;

  return sourceSteps.map((step, index) => {
    const fallback = DEFAULT_PAYMENT_STEPS[index] || DEFAULT_PAYMENT_STEPS[DEFAULT_PAYMENT_STEPS.length - 1];
    return normalizePaymentStep(step, fallback);
  });
}
...
class PaymentModal {
  ...
  async loadPaymentMethods() {
    try {
      ...
      const snapshot = await getDocs(q);
      this.methods = snapshot.docs.map((snapshotDoc) => {
        const data = snapshotDoc.data() || {};
        return {
          id: snapshotDoc.id,
          ...data,
          steps: resolveMethodSteps(data)
        };
      });
  ...
  formatPrice(price) {
    return new Intl.NumberFormat('fr-FR', { 
      style: 'currency', 
      currency: this.options.currency || 'HTG',
    ...
    return `
      <div style="text-align: center; padding: 1rem 0;">
        ...
        <p style="font-size: 0.9rem; color: #8B7E6B;">
          ...
          Vous pourrez suivre le statut de votre demande dans votre espace
        </p>
  ...
  async saveOrder(proofName) {
    try {
      ...
      const orderData = {
        ...
        currency: this.options.currency || 'HTG',
        orderType: this.options.orderType || 'order',
        ...
        proofName: proofName,
        proofImageName: this.proofImageFile?.name || '',
